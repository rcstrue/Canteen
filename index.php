...
$pages = ['dashboard','meals','purchases','outward','stock','suppliers','ingredients','recipes','expenses','budgets','users','audit'];
...
<head>
    ...
    <style>
        ...
        .report-card{background:linear-gradient(135deg,#fff 0%,#fafafa 100%);border:1px solid #e5e7eb}
        .stat-big{font-size:1.5rem;font-weight:700;line-height:1.2}
    </style>
</head>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-200 shrink-0">
        ...
        <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
            ...
            <a href="#" onclick="showPage('recipes')" class="sidebar-link<?php echo $page==='recipes'?' active':''; ?> flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium" data-page="recipes"><span>📋</span> Recipes</a>
    ...
    <main class="flex-1 overflow-y-auto">
        ...
        <div class="p-4 md:p-6 lg:p-8">
            <?php
$activePage = $page;
$allPages = ['dashboard','meals','purchases','outward','stock','suppliers','ingredients','recipes','expenses','budgets','users','audit'];
...
function showPage(page) {
    ...
    toggleSidebar(false);
    const loaders = { dashboard: loadDashboard, meals: loadMeals, purchases: loadPurchases, outward: loadOutward, stock: loadStock, suppliers: loadSuppliers, ingredients: loadIngredients, recipes: loadRecipes, expenses: loadExpenses, budgets: loadBudgets, users: loadUsers, audit: loadAudit };
...
async function loadDashboard() {
    const month = document.getElementById('dash-month')?.value || new Date().toISOString().slice(0, 7);
    const [s, pmr] = await Promise.all([api('get_monthly_report', { month }), api('get_per_meal_report', { month })]);
    const monthLabel = new Date(month + '-01').toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
    document.getElementById('dash-label').textContent = 'Dashboard — ' + monthLabel;
    ...
    // Per Meal Cost Report Table
    const pmTbody = document.getElementById('d-per-meal-table');
    const t = pmr.totals;
    pmTbody.innerHTML = pmr.data.map(d => {
        ...
        return `<tr class="border-t ${isToday ? 'bg-amber-50' : ''} ${!hasData ? 'text-gray-300' : ''}">
            <td class="px-3 py-1 text-sm">${d.date.slice(8)} <span class="text-gray-400 text-xs">${dayName}</span></td>
            <td class="px-2 py-1 text-center text-sm">${d.breakfast || ''}</td>
            <td class="px-2 py-1 text-center text-sm">${d.lunch || ''}</td>
            <td class="px-2 py-1 text-center text-sm">${d.dinner || ''}</td>
            <td class="px-2 py-1 text-right text-xs">${d.breakfast_cost ? INR(d.breakfast_cost) : ''}</td>
            <td class="px-2 py-1 text-right text-xs">${d.lunch_cost ? INR(d.lunch_cost) : ''}</td>
            <td class="px-2 py-1 text-right text-xs">${d.dinner_cost ? INR(d.dinner_cost) : ''}</td>
            <td class="px-2 py-1 text-right text-xs font-medium ${cpm > 0 ? 'text-amber-700' : ''}">${cpm > 0 ? INR2(cpm) : ''}</td>
        </tr>`;
    }).join('');
    document.getElementById('d-per-meal-totals').innerHTML = `<tr class="border-t-2 border-gray-800 bg-gray-100 font-bold">
        <td class="px-3 py-2 text-sm">TOTAL</td>
        <td class="px-2 py-1 text-center text-sm">${t.breakfast}</td>
        <td class="px-2 py-1 text-center text-sm">${t.lunch}</td>
        <td class="px-2 py-1 text-center text-sm">${t.dinner}</td>
        <td class="px-2 py-1 text-right text-sm">${t.breakfast_cost ? INR(t.breakfast_cost) : ''}</td>
        <td class="px-2 py-1 text-right text-sm">${t.lunch_cost ? INR(t.lunch_cost) : ''}</td>
        <td class="px-2 py-1 text-right text-sm">${t.dinner_cost ? INR(t.dinner_cost) : ''}</td>
        <td class="px-2 py-1 text-right text-sm text-amber-700">${t.cost_per_meal > 0 ? INR2(t.cost_per_meal) : ''}</td>
    </tr>`;
...
async function loadMeals() {
    const month = document.getElementById('meal-month')?.value || new Date().toISOString().slice(0, 7);
    const data = await api('get_meal_counts', { month });
    const monthLabel = new Date(month + '-01').toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
    document.getElementById('meal-label').textContent = 'Meal Count — ' + monthLabel;
    ...
    tbody.innerHTML = data.map(d => {
        ...
        const dayName = dateObj.toLocaleDateString('en-IN', { weekday: 'short' });
        const isToday = d.date === new Date().toISOString().slice(0, 10);
        return `<tr class="border-t ${isToday ? 'bg-amber-50' : ''}">
            <td class="px-3 py-1.5 text-sm">${d.date.slice(8)}<span class="text-gray-400 ml-1">${dayName}</span></td>
            <td class="px-2 py-1.5"><input type="number" min="0" class="w-full border rounded px-2 py-1 text-sm text-center" data-date="${d.date}" data-meal="breakfast" value="${d.breakfast || ''}"></td>
            <td class="px-2 py-1.5"><input type="number" min="0" class="w-full border rounded px-2 py-1 text-sm text-center" data-date="${d.date}" data-meal="lunch" value="${d.lunch || ''}"></td>
            <td class="px-2 py-1.5"><input type="number" min="0" class="w-full border rounded px-2 py-1 text-sm text-center" data-date="${d.date}" data-meal="dinner" value="${d.dinner || ''}"></td>
        </tr>`;
    }).join('');
...
function clearPurchaseForm() {
    ...
    addPurchaseRow(); updatePurchaseTotals();
}
...
function clearOutwardForm() {
    ...
    addOutwardRow(); updateOutwardTotals();
}

async function saveOutward() {
    const desc = document.getElementById('o-desc').value.trim(); if (!desc) return toast('Enter description', 'error');
    const items = [];
    document.querySelectorAll('#o-items > div:not(:first-child)').forEach(row => {
...
async function loadStock() {
    const res = await api('get_stock', { search: document.getElementById('s-search')?.value, category: document.getElementById('s-category')?.value, low_stock: document.getElementById('s-low-only')?.checked ? 'true' : '' });
...
async function loadRecipes() {
    const res = await api('list_recipes');
    const tbody = document.getElementById('recipe-list');
    if (!res?.length) { tbody.innerHTML = '<tr><td colspan="5" class="text-center py-8 text-gray-400">No recipes yet</td></tr>'; return; }
    tbody.innerHTML = res.map(r => {
        const ingList = (r.ingredients || []).map(i => `${i.qty} ${i.ingredient_name}`).join(', ');
        return `<tr class="border-t hover:bg-gray-50">
            <td class="px-4 py-2"><div class="font-medium">${r.name}</div><div class="text-xs text-gray-400 mt-0.5">${r.description||''} ${r.servings>1?'— '+r.servings+' servings':''}</div></td>
            <td class="px-4 py-2"><span class="px-2 py-0.5 rounded text-xs bg-gray-100">${r.meal_type}</span></td>
            <td class="px-4 py-2 text-center text-xs text-gray-500">${r.ingredient_count||0}</td>
            <td class="px-4 py-2 text-right font-medium">${INR(r.total_cost)}</td>
            <td class="px-4 py-2 text-center"><button onclick="viewRecipe(${r.id})" class="text-blue-600 text-xs mr-2">View</button><button onclick="deleteRecipe(${r.id})" class="text-red-500 text-xs">Delete</button></td>
        </tr>`;
    }).join('');
}
...
async function viewRecipe(id) {
    ...
    const ingRows = (r.ingredients || []).map(i => `<tr class="border-t"><td class="px-4 py-2">${i.ingredient_name}</td><td class="px-4 py-2 text-right">${i.qty} ${i.unit||''}</td><td class="px-4 py-2 text-right">${INR(i.unit_cost)}</td><td class="px-4 py-2 text-right font-medium">${INR(i.total_cost)}</td><td class="px-4 py-2 text-center"><button onclick="deleteRecipeIng(${i.id},${id})" class="text-red-500 text-xs">✕</button></td></tr>`).join('');
    const addIngRow = `<tr class="border-t bg-gray-50"><td colspan="5" class="px-4 py-2"><div class="flex items-center gap-2"><select id="ri-ing" class="border rounded px-2 py-1 text-sm flex-1"><option value="">Select ingredient...</option>${ingredients.map(i=>`<option value="${i.id}">${i.name} (${i.unit})</option>`).join('')}</select><input type="number" id="ri-qty" step="0.01" min="0" placeholder="Qty" class="border rounded px-2 py-1 text-sm w-24"><button onclick="addRecipeIng(${id})" class="px-3 py-1 bg-amber-600 text-white text-xs rounded-lg">Add</button></div></td></tr>`;
    document.getElementById('edit-modal-title').textContent = r.name + ' — ' + r.meal_type;
    document.getElementById('edit-modal-body').innerHTML = `<p class="text-sm text-gray-500 mb-3">${r.description||'No description'} | ${r.servings} servings | Cost: <strong>${INR(r.total_cost)}</strong></p><table class="w-full text-sm mb-2"><thead class="bg-gray-100"><tr><th class="text-left px-4 py-1 text-xs">Ingredient</th><th class="text-right px-4 py-1 text-xs">Qty</th><th class="text-right px-4 py-1 text-xs">Unit Cost</th><th class="text-right px-4 py-1 text-xs">Total</th><th class="px-4 py-1"></th></tr></thead><tbody>${ingRows}${addIngRow}</tbody></table>`;
    document.getElementById('edit-modal').classList.remove('hidden');
}

function addRecipeModal() {
    document.getElementById('edit-modal-title').textContent = 'Add Recipe';
    document.getElementById('edit-modal-body').innerHTML = `<form onsubmit="event.preventDefault();addRecipe(this)"><div class="space-y-3"><div><label class="block text-xs font-medium mb-1">Name *</label><input name="name" required class="w-full border rounded px-3 py-2 text-sm"></div><div class="grid grid-cols-2 gap-3"><div><label class="block text-xs font-medium mb-1">Meal Type</label><select name="meal_type" class="w-full border rounded px-3 py-2 text-sm"><option>Breakfast</option><option selected>Lunch</option><option>Dinner</option></select></div><div><label class="block text-xs font-medium mb-1">Servings</label><input name="servings" type="number" min="1" value="1" class="w-full border rounded px-3 py-2 text-sm"></div></div><div><label class="block text-xs font-medium mb-1">Description</label><textarea name="description" rows="2" class="w-full border rounded px-3 py-2 text-sm"></textarea></div></div><div class="flex justify-end gap-2 mt-4"><button type="button" onclick="document.getElementById('edit-modal').classList.add('hidden')" class="px-4 py-2 text-sm border rounded-lg">Cancel</button><button type="submit" class="px-4 py-2 text-sm bg-amber-600 text-white rounded-lg">Add</button></div></form>`;
    document.getElementById('edit-modal').classList.remove('hidden');
}

async function addRecipe(form) {
    const data = Object.fromEntries(new FormData(form));
    const res = await api('add_recipe', data);
    if (res.success) { toast('Recipe added'); document.getElementById('edit-modal').classList.add('hidden'); loadRecipes(); } else toast(res.error || 'Failed', 'error');
}

async function addRecipeIng(recipeId) {
    const ingId = document.getElementById('ri-ing').value;
    const qty = document.getElementById('ri-qty').value;
    if (!ingId || !qty) return toast('Select ingredient and qty', 'error');
    const res = await api('add_recipe_ingredient', { recipe_id: recipeId, ingredient_id: ingId, qty });
    if (res.success) { toast('Added'); viewRecipe(recipeId); } else toast(res.error||'Failed','error');
}

async function deleteRecipeIng(riId, recipeId) {
    const res = await api('delete_recipe_ingredient', { id: riId });
    if (res.success) { viewRecipe(recipeId); } else toast(res.error||'Failed','error');
}

async function deleteRecipe(id) { if (!confirm('Delete this recipe?')) return; const r = await api('delete_recipe', { id }); if (r.success) { toast('Deleted'); loadRecipes(); } else toast(r.error||'Failed','error'); }

// ═══ BUDGETS ══════════════════════════════════════════════════
async function loadBudgets() {
    const month = document.getElementById('bud-month')?.value || new Date().toISOString().slice(0, 7);
    ...
    tbody.innerHTML = res.map(r => {
        ...
        return `<tr class="border-t ${isTotal ? 'bg-gray-100 font-bold' : ''} ${overBudget ? 'bg-red-50' : ''}">
            <td class="px-4 py-2 font-medium">${r.category}</td>
            <td class="px-4 py-2 text-right">${INR(r.budget)}</td>
            <td class="px-4 py-2 text-right">${INR(r.actual)}</td>
            <td class="px-4 py-2 text-right ${r.variance < 0 ? 'text-red-600' : 'text-emerald-600'}">${r.variance < 0 ? '-' : ''}${INR(Math.abs(r.variance))}</td>
            <td class="px-4 py-2"><div class="flex items-center gap-2"><div class="flex-1 bg-gray-200 rounded-full h-2"><div class="${barColor} h-2 rounded-full" style="width:${Math.min(barWidth,100)}%"></div></div><span class="text-xs font-medium ${overBudget ? 'text-red-600' : ''}">${r.percent > 900 ? '—' : r.percent + '%'}</span></div></td>
        </tr>`;
    }).join('');
}
...
async function loadAudit() {
    ...
    if (!res?.length) { tbody.innerHTML = '<tr><td colspan="5" class="text-center py-8 text-gray-400">No audit logs</td></tr>'; return; }
